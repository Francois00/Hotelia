...
const TIPOS: { value: string; label: string; color: string }[] = [
  ...
  { value: 'TEMPORADA_BAJA',  label: 'Temporada Baja',   color: 'bg-blue-100 text-blue-800' },
  { value: 'FIN_DE_SEMANA',   label: 'Fin de Semana',    color: 'bg-purple-100 text-purple-800' },
  { value: 'EVENTO_ESPECIAL', label: 'Evento Especial',  color: 'bg-red-100 text-red-800' },
  { value: 'ESTADIA_LARGA',   label: 'Estadía Larga',    color: 'bg-green-100 text-green-800' },
]
...
export default function ReglasTarifaPage() {
  ...
  const ajusteDisplay = (n: number) => {
    const s = n >= 0 ? `+${n}%` : `${n}%`
    return <span className={n >= 0 ? 'text-green-600 font-semibold' : 'text-red-500 font-semibold'}>{s}</span>
  }

  const ajustePreview = form.ajuste_porcentaje
    ? parseFloat(form.ajuste_porcentaje) >= 0
      ? <span className="text-green-600 font-semibold">+{form.ajuste_porcentaje}%</span>
  ...
  return (
    <div className="p-6 max-w-7xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Reglas de Tarifa</h1>
        <p className="text-gray-500 text-sm mt-0.5">Configura ajustes automáticos de precio por temporada</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100">
            <h2 className="text-sm font-semibold text-gray-900">Reglas activas</h2>
          </div>
          {reglas.length === 0 ? (
            <div className="px-5 py-12 text-center text-sm text-gray-400">Sin reglas configuradas</div>
          ) : (
                  ...
                  {reglas.map(r => (
                    <tr key={r.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        ...
                        >
                          <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white transition-transform ${r.activo ? 'translate-x-4' : 'translate-x-1'}`} />
                        </button>
                      </td>
                      <td className="px-4 py-3">
                        <button
        ...
        {/* Formulario */}
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-200 shadow-sm p-5 space-y-4 h-fit">
          <h2 className="text-sm font-semibold text-gray-900">Nueva regla</h2>
          ...
          {needsNoches(form.tipo) && (
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Noches mínimas *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Ajuste % *</label>
