        ...
        return (
          <div key={n} className="flex items-center flex-1">
            <div className={`flex flex-col items-center min-w-0 ${n < steps.length ? 'flex-1' : ''}`}>
              ...
              <span
                onClick={() => done && onGo(n)}
                className={`text-xs mt-1 font-medium ${active ? 'text-blue-600' : done ? 'text-green-600 cursor-pointer hover:underline' : 'text-gray-400'}`}
              >
                {label}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
          ...
          {data.habitacion && (
            <div className="border border-green-300 bg-green-50 rounded-xl p-4 space-y-3">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">Precio por noche (editable)</label>
                ...
                {data.habitacion.tarifa_sugerida_hoy && (
                  <p className="text-xs text-gray-500 mt-0.5">Sugerido por IA: {formatCurrency(data.habitacion.tarifa_sugerida_hoy)}</p>
                )}
              </div>
        ...
        <div>
            ...
            <div className="flex border-b border-gray-200 mb-4">
              {(['mapa', 'lista'] as const).map(v => (
                <button key={v} onClick={() => setSubView(v)}
                  className={`px-4 py-2 text-sm font-medium border-b-2 -mb-px capitalize transition-colors ${subView === v ? 'border-blue-600 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                  {v === 'mapa' ? 'Mapa' : 'Lista'}
                </button>
              ))}
            </div>
          ...
          {!loading && buscado && disponibles.length === 0 && (
            <div className="text-center py-12 text-gray-400 text-sm">Sin habitaciones disponibles para estas fechas</div>
          )}
          ...
          {!loading && disponibles.length > 0 && subView === 'mapa' && (() => {
            ...
            return (
              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                {pisos.map(piso => (
                  <div key={piso} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                    <div className="px-4 py-2 bg-gray-50 border-b border-gray-200">
                      <span className="text-xs font-semibold text-gray-600 uppercase">Piso {piso}</span>
                    </div>
                      ...
                      {disponibles.filter(h => h.piso === piso).map(h => {
                        ...
                        return (
                          <button key={h.id} onClick={() => seleccionar(h)}
                            ...
                            className={`px-3 py-2 rounded-lg text-xs font-bold border-2 transition-all hover:scale-105 ${selected ? 'bg-blue-600 border-blue-600 text-white' : 'bg-green-50 border-green-400 text-green-800 hover:bg-green-100'}`}>
                            {h.numero}
                          </button>
                        )
                      })}
              ...
              {disponibles.map(h => {
                ...
                return (
                  <div key={h.id}
                    ...
                    className={`border-2 rounded-xl p-4 cursor-pointer transition-all hover:shadow-md ${selected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-bold text-gray-900">Hab. {h.numero} — {h.tipo}</span>
                      <span className="text-xs text-gray-500">Piso {h.piso}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-sm font-semibold text-gray-900">{formatCurrency(h.tarifa_base)}/noche</span>
                        {ia && ia !== h.tarifa_base && (
                          <span className={`ml-2 text-xs font-medium ${ia > h.tarifa_base ? 'text-green-600' : 'text-orange-500'}`}>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 text-xs font-medium rounded-lg ${selected ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
...
function Paso2({
  ...
  const bannerColor = banner?.tipo === 'verde' ? 'bg-green-50 border-green-300 text-green-800'
    : banner?.tipo === 'azul' ? 'bg-blue-50 border-blue-300 text-blue-800'
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="text" value={data.nombres} onChange={e => !readOnly && onUpdate({ nombres: e.target.value })}
            readOnly={readOnly}
            className={`w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${readOnly ? 'bg-gray-50' : ''}`} />
        </div>
        <div>
      ...
      <div className="flex justify-between">
        <button onClick={onBack} className="px-5 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">← Volver</button>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-gray-600">Hab. {data.habitacion?.numero} · {data.nombres} {data.apellidos}</p>
            <p className="text-xs text-gray-500">{data.fecha_entrada} → {data.fecha_salida}</p>
          </div>
        ...
        {data.pagos.map((p, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <input type="text" value={p.referencia} onChange={e => updatePago(i, 'referencia', e.target.value)}
              ...
              className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            {data.pagos.length > 1 && (
              <button onClick={() => removePago(i)} className="text-gray-400 hover:text-red-500 text-lg leading-none px-1">&times;</button>
            )}
          </div>
        ))}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className={`font-semibold ${Math.abs(diff) < 0.01 ? 'text-green-600' : diff < 0 ? 'text-red-500' : 'text-yellow-600'}`}>
            {formatCurrency(suma)}
          </span>
        </div>
        {Math.abs(diff) > 0.01 && (
      ...
      {hayEfectivo && (
        <div className="border border-gray-200 rounded-xl p-4 space-y-2">
          ...
          <div className="flex items-center gap-3">
            ...
            {monto_recibido && (
              <span className={`text-sm font-semibold ${vuelto >= 0 ? 'text-green-600' : 'text-red-500'}`}>
      ...
      {overrideConfirm && suma > data.total + 0.01 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 flex items-center justify-between">
          <p className="text-sm text-yellow-800">El monto supera el total. ¿Es correcto?</p>
    ...
    <div className="space-y-6">
      <div className="space-y-3">
        {[
          ...
          <label key={opt.value} className="flex items-center gap-3 cursor-pointer p-4 border-2 rounded-xl hover:bg-gray-50 transition-colors">
      ...
      {data.tipo_comprobante === 'factura' && (
        <div className="border border-gray-200 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">Email factura</label>
...
function Paso5({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="border border-gray-200 rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-gray-100">
          <div className="px-5 py-4">
            <p className="text-xs font-semibold text-gray-400 uppercase mb-2">🏠 Habitación</p>
            <p className="text-sm font-medium text-gray-900">Hab. {data.habitacion?.numero} — {data.habitacion?.tipo} · Piso {data.habitacion?.piso}</p>
          </div>
          <div className="px-5 py-4">
            <p className="text-xs font-semibold text-gray-400 uppercase mb-2">👤 Huésped</p>
            <p className="text-sm font-medium text-gray-900">{data.nombres} {data.apellidos}</p>
            <p className="text-xs text-gray-500">{data.tipo_documento}: {data.numero_documento}</p>
            {data.telefono && <p className="text-xs text-gray-500">📱 {data.telefono}</p>}
          </div>
          <div className="px-5 py-4">
            <p className="text-xs font-semibold text-gray-400 uppercase mb-2">📅 Fechas</p>
            <p className="text-sm text-gray-900">Entrada: {format(new Date(data.fecha_entrada), "EEEE d 'de' MMMM 'de' yyyy", { locale: es })}</p>
            <p className="text-sm text-gray-900">Salida: {format(new Date(data.fecha_salida), "EEEE d 'de' MMMM 'de' yyyy", { locale: es })}</p>
            <p className="text-sm text-gray-700 font-medium mt-1">Estadía: {data.numero_noches} noches · Check-out a las 12:00</p>
          </div>
          <div className="px-5 py-4">
            <p className="text-xs font-semibold text-gray-400 uppercase mb-2">💰 Pago</p>
            <p className="text-sm text-gray-600">{data.numero_noches} noches × {formatCurrency(data.precio_por_noche)} = <span className="font-bold text-gray-900">{formatCurrency(data.total)}</span></p>
            {data.pagos.map((p, i) => (
              <p key={i} className="text-sm text-gray-600">{METODOS.find(m => m.value === p.metodo)?.label}: {formatCurrency(parseFloat(p.monto) || 0)}</p>
            ))}
          </div>
          <div className="px-5 py-4">
            <p className="text-xs font-semibold text-gray-400 uppercase mb-2">🧾 Comprobante</p>
            <p className="text-sm text-gray-900 capitalize">{data.tipo_comprobante} de venta</p>
            {data.canal_envio.length > 0 && <p className="text-xs text-gray-500">Envío: {data.canal_envio.join(', ')}</p>}
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-between">
        <button onClick={onBack} className="px-5 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">← Modificar</button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-8 text-center space-y-4">
        <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto text-3xl animate-bounce">✅</div>
        ...
        <div className="space-y-1 text-sm text-gray-600">
          ...
          <p className="text-xs text-gray-400 mt-1">Comprobante: enviando...</p>
        </div>
        <div className="space-y-2 pt-2">
          <button onClick={handlePrint} className="w-full px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">
            🖨️ Imprimir voucher
          </button>
...
export default function CheckinManualPage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Check-in Manual</h1>
        <p className="text-gray-500 text-sm mt-0.5">Registra el ingreso de un huésped</p>
      </div>
